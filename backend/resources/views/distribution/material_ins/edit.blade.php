...
@section('title', 'Edit Bahan Masuk')
            ...
            <form action="{{ route('material_ins.update', $material_in->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="card-body">
                    <div class="form-group">
                        ...
                        <select class="form-control select2" id="ingredient_id" name="ingredient_id" required>
                            ...
                            @foreach($ingredients as $ingredient)
                                <option value="{{ $ingredient->id }}" {{ old('ingredient_id', $material_in->ingredient_id) == $ingredient->id ? 'selected' : '' }}>
                                    {{ $ingredient->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <input type="number" class="form-control" id="quantity" name="quantity" placeholder="Masukkan Jumlah" value="{{ old('quantity', $material_in->quantity) }}" required>
                                @error('quantity')
                                    <small class="text-danger">{{ $message }}</small>
                                @enderror
                            </div>
                        ...
                        <div class="col-md-2">
                            <div class="form-group">
                                ...
                                <input type="text" class="form-control" id="unit" name="unit" placeholder="Satuan" value="{{ old('unit', $material_in->unit) }}" disabled>
                            </div>
                        </div>
                            ...
                            <div class="form-group">
                                ...
                                <div class="input-group" id="in_date_picker" data-td-target-input="nearest" data-td-target-toggle="nearest">
                                    <input type="text" class="form-control" id="in_date_display" 
                                        ...
                                        placeholder="" 
                                        value="{{ old('in_date') ? \Carbon\Carbon::parse(old('in_date'))->format('d/m/Y') : \Carbon\Carbon::parse($material_in->in_date)->format('d/m/Y') }}" required readonly>
                                    <input type="hidden" id="in_date" name="in_date" value="{{ old('in_date', $material_in->in_date) }}">
                    ...
                    <div class="form-group">
                        ...
                        <textarea class="form-control" id="note" name="note" rows="3" placeholder="Masukkan catatan">{{ old('note', $material_in->note) }}</textarea>
                        @error('note')
                            <small class="text-danger">{{ $message }}</small>
                        @enderror
                    </div>
